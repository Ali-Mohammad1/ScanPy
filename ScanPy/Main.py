...
def scan_ports_multiple_hosts(logger, hosts: list, ports: list, timeout: int, max_workers: int, verbose: bool) -> dict:
    return scan_multiple_hosts(logger, hosts, ports, timeout, max_workers, verbose)
...
def main():
    ...
    if args.network != "192.168.1.0/24" and not args.scan_hosts:
    ...
    # تحديد range_of_hosts بقيمة افتراضية إن كانت None
    host_range = args.host_range if args.host_range is not None else 254

    if args.scan_hosts and args.scan_ports:
        logger.info("Both host scanning and port scanning requested. Scanning hosts first...")
        scanned_hosts = scan_hosts(logger, args.network, args.packet_count, host_range, args.timeout, args.verbose)
        if scanned_hosts:
